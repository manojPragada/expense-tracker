...
- **Recurring Payments**: Set up recurring expenses and incomes with flexible frequencies (daily, weekly, bi-weekly, monthly, yearly)
- **Auto-Generated Entries**: Automatic generation of recurring transaction entries via scheduled jobs
- **Subscription Management**: Cancel recurring subscriptions from any entry (parent or child)
...
1. **Create Recurring Payment**:
   - Click **Submit Income/Expense**
   - Fill in the transaction details
   - Check **"Recurring Payment"** checkbox
   - Select frequency (Daily, Weekly, Bi-Weekly, Monthly, Yearly)
   - Optionally set an end date (leave empty for indefinite)
   - Submit the form

2. **Cancel Recurring Subscription**:
   - Go to **View Submissions**
   - Find any entry from the recurring series (parent or child)
   - Click **"Cancel Subscription"** button
   - Confirm the cancellation

3. **Generate Recurring Entries**:
   - The system automatically generates recurring entries daily via scheduled job
   - For manual generation, run:
     ```bash
     php artisan recurring:generate
     ```

**Note**: Recurring entries are automatically generated by a scheduled job that runs daily. See [Deployment](#deployment) section for setting up the scheduler in production.
...
### Setting Up Scheduled Tasks (Recurring Payments)

For automatic generation of recurring entries, Laravel's task scheduler must run. Add this cron entry:

```bash
# Edit crontab
crontab -e

# Add this line (runs every minute, Laravel handles the scheduling)
* * * * * cd /path-to-your-project && php artisan schedule:run >> /dev/null 2>&1
```

**For local development/testing**, you can run the scheduler manually:

```bash
# Run scheduler (keeps running in terminal)
php artisan schedule:work

# Or generate recurring entries manually
php artisan recurring:generate
```

**Important**: The `recurring:generate` command is scheduled to run daily (configured in `routes/console.php`). The cron job runs Laravel's scheduler every minute, which then executes the recurring generation command when due.
