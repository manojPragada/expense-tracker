...
export default function AuthenticatedLayout({ header, children }) {
    ...
    return (
        <div className="min-h-screen bg-gray-100 dark:bg-gray-900">
            ...
            {/* Sidebar */}
            <div className={`fixed inset-y-0 left-0 z-50 bg-white dark:bg-gray-800 border-r-2 border-gray-300 dark:border-gray-700 shadow-xl transform transition-all duration-300 ease-in-out ${
                sidebarOpen ? 'translate-x-0' : '-translate-x-full'
            } lg:translate-x-0 ${sidebarCollapsed ? 'lg:w-20' : 'lg:w-64'} w-64 overflow-visible`}>
                <div className="flex flex-col h-full">
                    {/* Collapse Toggle */}
                    <div className={`flex items-center h-16 border-b-2 border-gray-300 dark:border-gray-700 ${sidebarCollapsed ? 'justify-center px-4' : 'justify-end px-6'}`}>
                        {/* Collapse button for desktop */}
                        {!sidebarCollapsed && (
                            <h2 className="hidden lg:block text-xl font-bold text-gray-900 dark:text-white text-left w-full py-4">Expense Tracker</h2>
                        )}
                        <button
                        ...
                        {navigation.map((item) => {
                            ...
                            return (
                                ...
                                >
                                    <Link
                                        href={item.href}
                                        className={`flex items-center ${sidebarCollapsed ? 'lg:justify-center' : 'space-x-3'} px-4 py-3 rounded-lg transition-all duration-200 ${
                                            active
                                    ...
                                    >
                                        <span className={`flex-shrink-0 ${active ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-500 dark:text-gray-400'}`}>
                                            {item.icon}
                                        </span>
                                        {!sidebarCollapsed && (
                    ...
                    <div className="px-0 py-3 border-t-2 border-gray-300 dark:border-gray-700">
                        <button
                            ...
                            className={`flex items-center w-full ${sidebarCollapsed ? 'lg:justify-center' : 'justify-between'} w-full px-4 py-3 rounded-lg text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors duration-200 border-2 border-transparent hover:border-gray-300 dark:hover:border-gray-600`}
                        ...
                        >
                            <div className={`flex items-center ${sidebarCollapsed ? 'lg:justify-center' : 'space-x-3'}`}>
                                ...
                                {!sidebarCollapsed && (
                                    <span className="font-medium hidden lg:block">{darkMode ? 'Light Mode' : 'Dark Mode'}</span>
                                )}
                                <span className="font-medium lg:hidden">{darkMode ? 'Light Mode' : 'Dark Mode'}</span>
                            </div>
                            {!sidebarCollapsed && (
                                <div className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200 hidden lg:flex ${
                            ...
                            <div className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200 lg:hidden ${
                                darkMode ? 'bg-indigo-600' : 'bg-gray-300'
                            }`}>
                                <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform duration-200 ${
                                    darkMode ? 'translate-x-6' : 'translate-x-1'
                    ...
                    <div className="border-t-2 border-gray-300 dark:border-gray-700 py-2 px-3">
                        <div className="space-y-2">
                            ...
                            <button 
                                ...
                                className={`flex items-center w-full rounded-lg text-left hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors duration-200 border-2 border-transparent hover:border-gray-300 dark:hover:border-gray-600 ${sidebarCollapsed ? 'lg:justify-center' : ''}`}
                            >
                                <div className={`flex items-center ${sidebarCollapsed ? 'lg:justify-center' : 'space-x-3 flex-1'}`}>
                                    ...
                                    {!sidebarCollapsed && (
                                        <div className="w-full flex items-center py-2">
                                            ...
                                            <ChevronDown className={`w-5 h-5 text-gray-400 dark:text-gray-500 ml-auto transition-transform duration-200 ${hoveredItem === 'user' ? 'rotate-180' : ''}`} />
                                        </div>
                                    )}
            ...
            {hoveredItem && sidebarCollapsed && typeof document !== 'undefined' && createPortal(
                <div 
